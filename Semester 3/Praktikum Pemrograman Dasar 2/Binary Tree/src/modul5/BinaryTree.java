...
public class BinaryTree {
    ...
    void delete(BinaryTreeNode deleted) {
        if (this.root != null) {
            if (deleted.has_no_child()) {
                ...
                // ....(); // remove the most left double slash, then unset parent from the replacement
                // .....(deleted.left); // remove the most left double slash, then set left of the replacement with deleted.left
                // .....(deleted.right); // remove the most left double slash, then set right of the replacement with deleted.right
                if (deleted == this.root) {
                    this.root = replacement;
                } else if (deleted.is_left()) {
                    deleted.parent.set_left(replacement);
    ...
    void findNode(int key) {
        if (this.root == null) {
            ...
            while (current != null) {
                if (key == current.data) {
                    System.out.println("Yey angka yang dicari ada !");
                    break;
                }
                if (key > current.data) {
                   current = current.right;
...
//                        System.out.println("Yuhuuuu angka ketemu!!");
//                        break;
//                    } else {
//                    }
//                    if (current.data == key) {
//                        System.out.println("Horeee dapat angka nya");
//                        break;
//                    } else {
